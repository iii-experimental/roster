...
iii.registerFunction(
  ...
  async (input: { budget_id: string; estimated_cost_usd?: number; principal_id?: string }) => {
    ...
    // Persist if rollover or exemption prune mutated anything; check is on the
    // hot path, so only write when state actually changed.
...
iii.registerFunction(
  ...
  async (input: { budget_id: string; window?: 'day' | 'week' | 'month' | 'all' }) => {
    ...
    let cutoff = 0;
    if (window !== 'all') {
      cutoff = periodStart(window, now());
    }

    const relevant = logs.filter((l) => l.period_start >= cutoff);
    // Include the current in-flight period so usage is not stale on fast reads.
...
iii.registerFunction('budget::forecast', async (input: { budget_id: string }) => {
  ...
  const rate = b.spent_usd / elapsed; // usd/day within current period

  // Projected for a 30-day month window (stable number for ops dashboards).
